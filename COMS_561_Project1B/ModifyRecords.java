...
//		portNum = result[3];
		...
		try {
			...
			System.out.print("--- updating student name to Scott whose ssn = 746897816 ---\n");
			updateSQL(conn, "update students\r\n"
					+ "set	name = 'Scott'\r\n"
					+ "where ssn = 746897816;");
			
			//modification 2
			//Change the major of the student with ssn = 746897816 to Computer Science, Master
			System.out.print("--- updating major to Computer Science, Master, whose ssn = 746897816 ---\n");
			updateSQL(conn, "update major\r\n"
			...
			System.out.print("--- Deleting all registration records that were in “Spring2021” ---\n");
			updateSQL(conn, "set SQL_SAFE_UPDATES = 0;");
			updateSQL(conn, "delete \r\n"
					+ "from register \r\n"
					+ "where regtime = 'Spring2021';");
			updateSQL(conn, "set SQL_SAFE_UPDATES = 1;");
